...
public class Delete extends JDialog implements ActionListener {
	...
	public Delete() {
		...
		lblPhone.setForeground(new Color(128, 0, 128));
		...
		lblBalance.setForeground(new Color(128, 0, 128));
		...
		load.setBackground(new Color(128, 0, 128));
		...
		delete.setBounds(248, 308, 89, 23);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ob==load){
			...
			iD=Integer.parseInt(id.getText());
			String st="delete from information where ID='"+id.getText()+"'";  
			Connection cc=Database.connect();
			try{
				PreparedStatement ps=cc.prepareStatement(st);
				
			ps.executeUpdate();
			JOptionPane.showMessageDialog(this, "information delete");
			dispose();
		}
		catch(SQLException se){
			se.printStackTrace();
